...
solve m0 = genericSolve sn neighbours expanded where
  ...
  expanded = let (P _ ymax xmax) = maximum $ M.keys m0 in M.fromList
    [ let p = P z y x in (p, M.findWithDefault Inactive p m0)
    | z <- [-sn..sn]
    , y <- [-sn..ymax+sn]
    , x <- [-sn..xmax+sn]
    ]
