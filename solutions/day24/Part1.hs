...
solve :: ([Army], [Army]) -> Int
solve (immuneSystem0, infection0) = runST $ do
  ...
  where
    ...
    -- battle two groups against each other, mutating the original
    -- vectors
    battle :: V.STVector s (Army, Maybe Int) -> V.STVector s (Army, Maybe Int) -> ST s Int
    battle group1 group2 = loop where
      loop = checkVictory >>= \case
        Just score -> pure score
        Nothing -> do
          targetSelection =<< epowerOrder
          attack
          loop
      ...
      checkVictory = do
        score1 <- score group1
        score2 <- score group2
        pure $ if | score1 == 0 -> Just score2
                  | score2 == 0 -> Just score1
                  | otherwise   -> Nothing
